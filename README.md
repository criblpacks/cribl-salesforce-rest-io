...
Some configuration items are required for pulling data from the Salesforce API. Navigate to Source > Collectors (REST) > cribl_salesforce within the pack to modify the config.

* Update the Salesforce domain - This will need to be updated in *3 places*: Discover > Discover URL, Collect > Collect URL, Authentication > Login URL
* Update the Salesforce API Version - This will need to be updated in *2 places*: Discover > Discover URL and Collect > Collect URL (note that it occurs *twice* in the Collect URL section)
* Update the Client Secret Value under Authentication
* Update the client_id value under Authentication

Optional Configuration:
- If additional objects from the Salesforce API are needed you can modify the JSON Array under Discover > Format discover results. Two objects have been configured to start and provide the formatting for how to add additional objects. For information on available objects and fields please reference [Salesforce Objects](https://developer.salesforce.com/docs/atlas.en-us.object_reference.meta/object_reference/sforce_api_objects_list.htm).
